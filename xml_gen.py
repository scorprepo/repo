...
class Generator:
    ...
    """
    def __init__(self):
# Create the addons folder if it doesn't already exist
        zips_path = ('addons')
...
# Comment out this line if you have .pyc or .pyo files you need to keep
    ...
    def _create_zips(self, addon_id, version):
        ...
        addon_folder = os.path.join('addons', addon_id)
        if not os.path.exists(addon_folder):
            os.makedirs(addon_folder)

        final_zip = os.path.join('addons', addon_id, '{0}-{1}.zip'.format(addon_id, version))
...
# Remove any instances of pyc or pyo files
    ...
    def _generate_addons_file(self):
        ...
        for addon in addons:
            try:
                if not os.path.isdir(addon) or addon == "addons" or addon.startswith('.'):
                    continue
        ...
        self._save_file(addons_xml.encode('utf-8'), file=os.path.join('addons', 'addons.xml'), decode=True)

    def _generate_md5_file(self):
        try:
            m = hashlib.md5(open(os.path.join('addons', 'addons.xml'), 'r', encoding='utf-8').read().encode('utf-8')).hexdigest()
            self._save_file(m, file=os.path.join('addons', 'addons.xml.md5'))
        except Exception as e:
