...
INCLUDEJOREL       = wiz.getS('includejorel')
INCLUDESCORPION   = wiz.getS('includescorpion')
INCLUDESEREN    = wiz.getS('includeseren')
INCLUDEEXODUSREDUX   = wiz.getS('includeexodusredux')
INCLUDEKRATOS    = wiz.getS('includekratos')
INCLUDECLOWNS   = wiz.getS('includeclowns')
...
def maintMenu(view=None):
	...
	if includeall == 'true':
		includejorel = 'true'
		includescorpion = 'true'
		includeseren = 'true'
		includeexodusredux = 'true'
		includekratos = 'true'
		includeclowns = 'true'
	else:
		includejorel = 'true' if INCLUDEJOREL     == 'true' else 'false'
		includescorpion = 'true' if INCLUDEYODA == 'true' else 'false'
		includeseren = 'true' if INCLUDESEREN  == 'true' else 'false'
		includeexodusredux = 'true' if INCLUDEEXODUSREDUX == 'true' else 'false'
		includekratos = 'true' if INCLUDEKRATOS  == 'true' else 'false'
		includeclowns = 'true' if INCLUDECLOWNS == 'true' else 'false'
	...
	if includevid == 'true':
		...
		addFile('--- Include Jor-el: %s' % includejorel.replace('true',on).replace('false',off), 'togglecache', 'includejorel', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Scorpion: %s' % includescorpion.replace('true',on).replace('false',off), 'togglecache', 'includescorpion', icon=ICONMAINT, themeit=THEME3)
		...
		addFile('--- Include The Kratos: %s' % includekratos.replace('true',on).replace('false',off), 'togglecache', 'includekratos', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include 13 Clowns: %s' % includeclowns.replace('true',on).replace('false',off), 'togglecache', 'includeclowns', icon=ICONMAINT, themeit=THEME3)
...
def toggleCache(state):
	cachelist = ['includevideo', 'includeall', 'includejorel', 'includescorpion', 'includeseren', 'includeexodusredux', 'includekratos', 'includeclowns']
	titlelist = ['Include Video Addons', 'Include All Addons', 'Include Jorel', 'Include Scorpion', 'Include Seren', 'Include Exodusredux', 'Include Kratos', 'Include clowns']
...
def speedMenu():
	xbmc.executebuiltin('Runscript("special://home/addons/plugin.program.onenationportal/speedtest.py")')
######################################################################################################################
