...
INCLUDEJOREL     = ADDON.getSetting('includejorel')
INCLUDESCORPION  = ADDON.getSetting('includescorpion')
INCLUDESEREN  = ADDON.getSetting('includeseren')
INCLUDEEXODUSREDUX = ADDON.getSetting('includeneexodusredux')
INCLUDECLOWNS  = ADDON.getSetting('includeclowns')
INCLUDEKRATOS = ADDON.getSetting('includekratos')
...
def getCacheSize():
	...
	dbfiles   = [
		(os.path.join(ADDONDATA, 'plugin.video.scorpion', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.jorel', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.seren', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.exodusredux', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.13clowns', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.kratos', 'cache.db')),
        (os.path.join(ADDONDATA, 'plugin.video.scorpion', 'cache.db')),
		(os.path.join(DATABASE,  'cache.db')),]
	cachelist = [
		(PROFILEADDONDATA),
		(ADDONDATA),
	...
	if INCLUDEVIDEO == 'true':
		...
		if INCLUDEALL == 'true': files = dbfiles
			...
			if INCLUDEDEATH == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.blamo', 'cache.db'))
			if INCLUDEDEATHRD == 'true': files.append(os.path.join(ADDONDATA, 'plugin.video.dsrdplus', 'cache.db'))
			if INCLUDEPLACE == 'true':  files.append(os.path.join(ADDONDATA, 'plugin.video.placenta', 'cache.db'))
			if INCLUDENEPTUNE == 'true': files.append(os.path.join(ADDONDATA, 'plugin.video.neptune', 'cache.db'))
			if INCLUDEKNOB == 'true':  files.append(os.path.join(ADDONDATA, 'plugin.video.knob', 'cache.db'))
			if INCLUDEPLACE == 'true': files.append(os.path.join(DATABASE,  'cache.db'))
			if INCLUDEDEATH == 'true':   files.append(os.path.join(DATABASE,  'DEATHScache.db'))
			if INCLUDEDEATHRD == 'true': files.append(os.path.join(DATABASE,  'DSRDcache.db'))
		if len(files) > 0:
...
def clearCache(over=None):
	PROFILEADDONDATA = os.path.join(PROFILE,'addon_data')
	dbfiles   = [
		(os.path.join(ADDONDATA, 'plugin.video.blamo', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.dsrdplus', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.placenta', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.neptune', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.knob', 'cache.db')),
	...
	if INCLUDEVIDEO == 'true' and over == None:
		...
		if INCLUDEALL == 'true': files = dbfiles
			...
			if INCLUDEPLACE == 'true': files.append(os.path.join(DATABASE,  'cache.db'))
			if INCLUDEDEATH == 'true':   files.append(os.path.join(DATABASE,  'DEATHScache.db'))
			if INCLUDEDEATHRD == 'true': files.append(os.path.join(DATABASE,  'DSRDcache.lite.db'))
		if len(files) > 0:
			for item in files:
