...
# -Cleaned and Checked on 04-14-2020 by Scorpion.
...
class source:
    ...
    def resolve(self, url):
        try:
            ...
            p1 = cfscrape.post('http://www6.123123movies.net/ip.file/swf/plugins/ipplugins.php', data=post, headers=self.headers).content
            p1 = json.loads(p1)
            p2 = cfscrape.get('http://www6.123123movies.net/ip.file/swf/ipplayer/ipplayer.php?u=%s&s=%s&n=0' % (p1['s'], urldata['data-server']), headers=self.headers).content
            p2 = json.loads(p2)
            p3 = cfscrape.get('http://www6.123123movies.net/ip.file/swf/ipplayer/api.php?hash=%s' % (p2['hash']), headers=self.headers).content
            p3 = json.loads(p3)
            n = p3['status']
            if n is False:
                p2 = cfscrape.get('http://www6.123123movies.net/ip.file/swf/ipplayer/ipplayer.php?u=%s&s=%s&n=1' % (p1['s'], urldata['data-server']), headers=self.headers).content
                p2 = json.loads(p2)
            url = p2["data"].replace("\/","/")
