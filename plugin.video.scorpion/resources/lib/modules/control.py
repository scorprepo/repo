...
def artPath():
    ...
    if theme in ['-', '']:
        return
    elif condVisibility('System.HasAddon(script.scorpion.artwork)'):
        return os.path.join(xbmcaddon.Addon(
            'script.scorpion.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting(
        'appearance.1').lower() if condVisibility(
        'System.HasAddon(script.scorpion.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.scorpion.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.scorpion.metadata)'):
        return os.path.join(xbmcaddon.Addon(
            'script.scorpion.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
