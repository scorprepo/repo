...
# Doko'S Dupe Checker for cleaning results.
            ...
            try:
                ...
                if len(self.sources) > 0:
                    if quality in ['0']:
                        ...
                        source_1080 = len(
                            [e for e in self.sources if e['quality'] == '1080p' and e['debridonly'] is False])
                        source_720 = len(
                            [e for e in self.sources if e['quality'] == '720p' and e['debridonly'] is False])
                    ...
                    if debrid_status:
                        if len(self.sources) > 0:
                            for d in debrid_list:
                                if quality in ['0']:
                                    d_source_4k = len([e for e in self.sources if
                                                       e['quality'] == '4K' and d.valid_url(e['url'], e['source'])])
                                    d_source_1080 = len([e for e in self.sources if
                                                         e['quality'] in '1080p' and d.valid_url(e['url'],
                                                                                                 e['source'])])
                                    d_source_720 = len([e for e in self.sources if
                                                        e['quality'] in '720p' and d.valid_url(e['url'], e['source'])])
                                    d_source_sd = len([e for e in self.sources if
                                                       e['quality'] == 'SD' and d.valid_url(e['url'], e['source'])])
                                elif quality in ['1']:
                                    d_source_1080 = len([e for e in self.sources if
                                                         e['quality'] in '1080p' and d.valid_url(e['url'],
                                                                                                 e['source'])])
                        ...
                        if debrid_status:
                            if quality in ['0']:
                                if not progressDialog == control.progressDialogBG:
                                    ...
                                    string6, d_4k_label, d_1080_label, d_720_label, d_sd_label, d_total_label)
                                ...
                                if not progressDialog == control.progressDialogBG:
                                    ...
                                    string6, d_1080_label, d_720_label, d_sd_label, d_total_label)
                                    line2 = ('%s:' + '|'.join(pdiag_format[1:])) % (
                                    string7, source_1080_label, source_720_label, source_sd_label, source_total_label)
                                else:
                                    line1 = '|'.join(pdiag_bg_format[1:]) % (
                                ...
                                if not progressDialog == control.progressDialogBG:
                                    ...
                                    string6, d_720_label, d_sd_label, d_total_label)
                                    line2 = ('%s:' + '|'.join(pdiag_format[2:])) % (
                                    string7, source_720_label, source_sd_label, source_total_label)
                                else:
                                ...
                                if not progressDialog == control.progressDialogBG:
                                    line1 = ('%s:' + '|'.join(pdiag_format[3:])) % (string6, d_sd_label, d_total_label)
                                    line2 = ('%s:' + '|'.join(pdiag_format[3:])) % (
                                    string7, source_sd_label, source_total_label)
                                else:
                            ...
                            if quality in ['0']:
                                ...
                                source_4k_label, source_1080_label, source_720_label, source_sd_label,
                                source_total_label)
                            elif quality in ['1']:
                                line1 = '|'.join(pdiag_format[1:]) % (
                                source_1080_label, source_720_label, source_sd_label, source_total_label)
                            elif quality in ['2']:
    ...
    def prepareSources(self):
        try:
            ...
            dbcur = dbcon.cursor()
            dbcur.execute(
                "CREATE TABLE IF NOT EXISTS rel_url (""source TEXT, ""imdb_id TEXT, ""season TEXT, ""episode TEXT,"
                " ""rel_url TEXT, ""UNIQUE(source, imdb_id, season, episode)"");")
            dbcur.execute(
                "CREATE TABLE IF NOT EXISTS rel_src (""source TEXT, ""imdb_id TEXT, ""season TEXT, ""episode TEXT,"
    ...
    def sourcesFilter(self):
        ...
        for d in debrid.debrid_resolvers:
            ...
            if sortthemup == 'true':
                ...
                filter += [dict(i.items() + [('debrid', d.name)]) for i in self.sources if
                           i['source'] in valid_hoster and 'magnet:' not in i['url']]
            else:
                filter += [dict(i.items() + [('debrid', d.name)]) for i in self.sources if
                           i['source'] in valid_hoster or 'magnet:' in i['url']]
        if debrid_only == 'false' or debrid.status() is False:
            filter += [i for i in self.sources if
                       not i['source'].lower() in self.hostprDict and i['debridonly'] is False]
        ...
        if quality in ['0']:
            filter += [i for i in self.sources if i['quality'] == '4K' and 'debrid' not in i]

        if quality in ['0', '1']:
            filter += [i for i in self.sources if i['quality'] == '1080p' and 'debrid' in i]
        if quality in ['0', '1']:
            filter += [i for i in self.sources if i['quality'] == '1080p' and 'debrid' not in i]

        if quality in ['0', '1', '2']:
            filter += [i for i in self.sources if i['quality'] == '720p' and 'debrid' in i]
        if quality in ['0', '1', '2']:
            filter += [i for i in self.sources if i['quality'] == '720p' and 'debrid' not in i]
        ...
        self.sources = self.sources[:3500]
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.scorpion.container.items'

        self.metaProperty = 'plugin.video.scorpion.container.meta'
        ...
        self.hostcapDict = ['openload.io', 'openload.co', 'oload.tv', 'oload.stream', 'oload.win', 'oload.download',
                            ...
                            'vev.io', 'vidup.me', 'vidup.tv', 'vidup.io', 'vshare.io', 'vshare.eu', 'flashx.tv',
                            'flashx.to', 'flashx.sx', 'flashx.bz', 'flashx.cc',
                            'hugefiles.net', 'hugefiles.cc', 'thevideo.me', 'streamin.to']

        self.hosthqDict = ['gvideo', 'google.com', 'thevideo.me', 'raptu.com', 'filez.tv', 'uptobox.com',
                           'uptostream.com',
                           'xvidstage.com', 'xstreamcdn.com', 'idtbox.com']

        self.hostblockDict = ['zippyshare.com', 'youtube.com', 'facebook.com', 'twitch.tv', 'streamango.com',
                              'streamcherry.com',
                              'openload.io', 'openload.co', 'openload.pw', 'oload.tv', 'oload.stream', 'oload.win',
                              'oload.download', 'oload.info', 'oload.icu', 'oload.fun', 'oload.life', 'oload.space',
                              'oload.monster',
                              'rapidvideo.com', 'rapidvideo.is', 'rapidvid.to']

        self.sourcecfDict = ['123123movies', '123movieshubz', 'extramovies', 'movie4kis', 'projectfree', 'rapidmoviez',
                             'rlsbb', 'scenerls', 'timewatch', 'tvmovieflix', '1337x', 'btdb', 'ytsam',
                             'animebase', 'filmpalast', 'hdfilme', 'iload', 'movietown', '1putlocker', 'animetoon',
                             'azmovie', 'cartoonhdto', 'cmoviestv', 'freefmovies', 'ganoolcam', 'projectfreetv',
                             'putlockeronl',
                             'sharemovies', 'solarmoviefree', 'tvbox', 'xwatchseries', '0day', '2ddl', 'doublr',
                             'pirateiro']
