...
if action is None:
    ...
    control.execute('ActivateWindow(favourites)')
    ...
    from resources.lib.modules import control
    control.execute('RunPlugin(plugin://script.module.resolveurl/?mode=auth_rd)')
...
if mode == "toggleAllNormal":
    sourcelist = ['5movies', '123123movies', 'anime1', 'animeram', 'animetoon', 'cartoonhd', 'cmovieshd',
                  'extramovies', 'filmxy', 'fmoviesio', 'ganool123', 'hdmo', 'icefilms','iwannawatch',
                  'newepisodes', 'projectfreetv', 'series9', 'seriesonline', 'seriz', 'showbox',
                  'swatchseries', 'toonova', 'timewatch', 'watchepisodes4', 'watchfree', 'watchseries',
                  'watchserieshd', 'watchseriessto', 'wsunblock', 'yesmoviesgg', 'zmovies']
    toggleAll(params['setting'], params['query'], sourcelist)

if mode == "toggleAllDebrid":
    sourcelist = ['300mbfilms', 'crazyhdsource', 'ganool', 'maxrls', 'mvrls', 'onceddl', 'rlsb', 'rlsbb']
    toggleAll(params['setting'], params['query'], sourcelist)
